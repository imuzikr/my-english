...
`manifest.json`과 `sw.js`는 서로 다른 규칙을 따릅니다.

**`manifest.json`** — 위치 제약 없음. 어떤 경로에 두든 `<link rel="manifest" href="...">`로 정확히 연결만 하면 됩니다.

**`sw.js`** — 자신이 등록된 경로의 하위 범위(scope)만 제어합니다.

| Service Worker 위치 | 제어 가능한 범위 |
|---|---|
| `/sw.js` | 사이트 전체 |
| `/app/sw.js` | `/app/` 하위만 |
| `/my-english/sw.js` | `/my-english/` 하위만 |

> 💡 GitHub Pages 프로젝트 사이트(`username.github.io/repo-name/`)처럼 서브패스로 배포할 경우, `sw.js`는 해당 서브패스의 루트(`/repo-name/sw.js`)에 두어야 그 안의 모든 페이지를 제어할 수 있습니다. 자세한 내용은 맨 아래 [GitHub Pages 배포 시 주의사항](#github-pages-배포-시-주의사항)을 참고하세요.
...
> ⚠️ **경로 표기 주의**: 아래 예시는 **사이트가 도메인 루트(`/`)에 배포된 경우**를 기준으로 합니다. GitHub Pages 프로젝트 사이트처럼 서브패스로 배포한다면 `/manifest.json` → `/repo-name/manifest.json`, `/sw.js` → `/repo-name/sw.js`로 바꿔야 합니다. 자세한 내용은 맨 아래 [GitHub Pages 배포 시 주의사항](#github-pages-배포-시-주의사항)을 참고하세요.
...
- [ ] `manifest.json`이 `<link rel="manifest" href="...">`로 정확히 연결됨 (위치는 어디든 무관)
- [ ] `sw.js`가 제어하려는 범위의 최상위 경로에 위치하고, 같은 경로로 `navigator.serviceWorker.register(...)` 등록됨
  - 도메인 루트 배포: `/sw.js` + `register('/sw.js')`
  - 서브패스 배포(`/app/` 등): `/app/sw.js` + `register('/app/sw.js')`
- [ ] `manifest.json`의 `start_url`과 `scope`가 실제 배포 경로와 일치함
...
저장소 이름이 있는 경우(`username.github.io/repo-name/`)는 모든 절대 경로(`/...`)가 도메인 루트가 아닌 **서브패스 기준**이 되어야 합니다. 그렇지 않으면 `/manifest.json`이 `https://username.github.io/manifest.json`으로 잘못 해석되어 404가 발생합니다.

### manifest 연결 경로

```html
<link rel="manifest" href="/repo-name/manifest.json">
<link rel="apple-touch-icon" href="/repo-name/icons/apple-touch-icon.png">
```

### `manifest.json`의 `start_url`과 `scope`
...
> 💡 `manifest.json` 안의 `icons[].src`는 **manifest 파일 자체의 위치를 기준으로 한 상대 경로**로 적으면 안전합니다. 예: manifest가 `/repo-name/manifest.json`이고 아이콘이 `/repo-name/icons/icon-192.png`라면 `"src": "icons/icon-192.png"`로 충분합니다.

### Service Worker 위치와 등록 경로

`sw.js`는 제어하고 싶은 범위의 최상위 경로(`/repo-name/sw.js`)에 두어야 합니다.
...
`sw.js` 내부에서 다른 사이트 자산까지 잘못 가로채지 않도록 경로 필터링도 권장합니다.

```js
// sw.js 내부 — scope 밖 요청은 무시
